...
class Http {
	...
	static async get(url, data: { [x: string]: unknown }) {
		let getReqStr = '?';
		...
		return await Http.request(RequestMethod.Get, `${url}${getReqStr}`, null);
	}

	static async post(url, data: { [x: string]: unknown } | FormData, isFileUpload) {
